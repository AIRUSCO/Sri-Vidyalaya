...
function doPost(e) {
    try {
        ...
        // Create file blobs for email attachments
        const attachments = [];
        
        if (e.parameter.aadhaarDocument) {
            try {
                const blob = Utilities.newBlob(e.parameter.aadhaarDocument, 'application/octet-stream', 'Aadhaar_' + data.aadhaar + '.pdf');
                attachments.push(blob);
            } catch (err) { Logger.log('Aadhaar attachment error: ' + err); }
        }
        
        if (e.parameter.studentPhoto) {
            try {
                const blob = Utilities.newBlob(e.parameter.studentPhoto, 'application/octet-stream', 'Photo_' + data.studentName + '.jpg');
                attachments.push(blob);
            } catch (err) { Logger.log('Photo attachment error: ' + err); }
        }
        
        if (e.parameter.marksheet) {
            try {
                const blob = Utilities.newBlob(e.parameter.marksheet, 'application/octet-stream', 'Marksheet_' + data.percentage + '.pdf');
                attachments.push(blob);
            } catch (err) { Logger.log('Marksheet attachment error: ' + err); }
        }
        
        if (e.parameter.transferCertificate) {
            try {
                const blob = Utilities.newBlob(e.parameter.transferCertificate, 'application/octet-stream', 'TC_' + data.admissionNo + '.pdf');
                attachments.push(blob);
            } catch (err) { Logger.log('TC attachment error: ' + err); }
        }
...
function saveFileToFolder(folder, blob, docType, reference) {
    try {
        ...
        const fileName = docType + '_' + reference + '_' + timestamp;
...
function sendSubmissionEmails(data, documentUrls, studentFolder, attachments) {
    ...
    try {
        ...
        <p><b>Attachments:</b> All documents are attached to this email.</p>
        `;
